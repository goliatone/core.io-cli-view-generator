...
function generateFromGUISchema(guiSchema, templates, target, saveGuiSchema = false) {
    ...
    if (saveGuiSchema) {
        saveFile(guiSchema, target);
    }
    console.log('generate gui schema');
...
function processModels(models, templates, target) {
    ...
    return Promise.all(out).catch(err => {
        console.error('Error processing models...');
        console.error(err);
        return err;
    });
}
...
function processModel(model, templates, target) {
    ...
    return Promise.all([
        ...
        console.info('pattern', Compiler.pattern);
