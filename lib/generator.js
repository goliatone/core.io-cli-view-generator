        ...
        files.map(file => {
            ...
            file.src.map(src => {
                ...
                if (typeof content === 'string') write(file.dest, content);
                    ...
                    logger.error('genrator: Error parsing content');
...
function saveFile(config) {
    let { schema, target, logger, filename } = config;
    ...
    logger.debug('Saving %s at path: %s', filename, filepath);
